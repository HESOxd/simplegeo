...
// Нормализация короткого ответа: ё=е, регистр, лишние пробелы и знаки игнорируем.
function norm(s) {
...
export default function App() {
  ...
  function isRight() {
    ...
    if (task.type === "single") return single === task.correct;
    if (task.type === "multi") {
      const a = [...multi].sort().join(",");
      const b = [...task.correct].sort().join(",");
      return a === b && multi.length > 0;
    }
    if (task.type === "short") return norm(text) === norm(task.answer);
    return false;
  }
  function check() {
    if (answered) return;
    ...
    return (
      <Shell>
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-slate-900 mt-1">SimpleGeo — тренажёр</h1>
