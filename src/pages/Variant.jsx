    ...
    return (
      <Shell>
        <div className="text-center mb-8">
          <p className="text-sm font-semibold uppercase tracking-wide text-emerald-700">Результат варианта</p>
        ...
        <div className="space-y-3">
          {answers.map((a, i) => (
            <div key={i} className={`rounded-xl border-2 p-4 ${a.right ? "border-emerald-200 bg-emerald-50/40" : "border-rose-200 bg-rose-50/40"}`}>
              <div className="flex items-start justify-between gap-3">
                <p className="text-sm font-medium text-slate-800">{i + 1}. {a.task.q}</p>
                <span className={`shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full ${a.right ? "bg-emerald-600 text-white" : "bg-rose-500 text-white"}`}>
                  {a.right ? "верно" : "неверно"}
                </span>
              </div>
              {!a.right && (
                <p className="text-sm text-slate-600 mt-2">
                  Правильный ответ: <b className="text-emerald-700">
        ...
        <div className="mt-8 flex gap-3">
          <button onClick={start} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 rounded-xl transition-colors">Новый вариант</button>
